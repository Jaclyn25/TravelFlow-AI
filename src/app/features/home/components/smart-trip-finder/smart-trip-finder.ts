    ...
    <div class="glass relative z-10 w-full max-w-xl mx-auto overflow-hidden rounded-[2.5rem] border-white/40 shadow-2xl dark:bg-navy-900/40">
      <div class="p-8 md:p-12 relative z-10">
        <div class="text-center mb-10">
          ...
          <div class="w-12 h-1 bg-teal-600 mx-auto rounded-full"></div>
        </div>
        ...
        @if (currentStep() < 4) {
          ...
          <div class="flex justify-between mb-12 px-2 relative">
             ...
             @for (step of [1, 2, 3]; track step) {
               <div [class]="'w-12 h-12 rounded-2xl flex items-center justify-center text-sm font-black transition-all duration-500 ' + (currentStep() >= step ? 'bg-navy-900 dark:bg-teal-600 text-white shadow-xl' : 'bg-white dark:bg-slate-800 border border-slate-100 dark:border-white/5 text-slate-300')">
                  {{ step }}
               </div>
             }
          </div>
              ...
              <div class="grid grid-cols-2 gap-4">
                @for (d of destinations; track d) {
                  <button (click)="selectDestination(d)" 
                          [class]="'py-5 px-6 rounded-2xl text-sm font-extrabold border-2 transition-all ' + (selection().destination === d ? 'border-teal-600 bg-teal-50 text-teal-600 dark:bg-teal-600/10' : 'border-slate-50 bg-slate-50/50 text-slate-500 hover:border-teal-100 dark:border-white/5 dark:bg-white/5')">
                    {{ d }}
                  </button>
                }
              </div>
              ...
              <div class="flex flex-col gap-4">
                @for (b of budgets; track b.value) {
                  <button (click)="selectBudget(b.value)"
                          [class]="'py-5 px-8 rounded-2xl text-left border-2 transition-all ' + (selection().budget === b.value ? 'border-teal-600 bg-teal-50 dark:bg-teal-600/10' : 'border-slate-50 bg-slate-50/50 hover:border-teal-100 dark:border-white/5 dark:bg-white/5')">
                    <div [class]="'font-extrabold text-sm ' + (selection().budget === b.value ? 'text-teal-600' : 'text-navy-900 dark:text-white')">{{ b.label }}</div>
          ...
          @if (currentStep() === 3) {
            <div class="animate-fade-in-up">
              <label class="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] mb-6 flex items-center gap-2">
                <svg class="w-4 h-4 text-teal-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.828 14.828a4 4 0 01-5.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                Experience Type
              </label>
              <div class="grid grid-cols-2 gap-4">
                @for (s of styles; track s.value) {
                  <button (click)="selectStyle(s.value)"
                          [class]="'py-8 px-4 rounded-3xl flex flex-col items-center gap-4 border-2 transition-all ' + (selection().style === s.value ? 'border-teal-600 bg-teal-50 text-teal-600 dark:bg-teal-600/10' : 'border-slate-50 bg-slate-50/50 text-slate-500 hover:border-teal-100 dark:border-white/5 dark:bg-white/5')">
          ...
          <div class="flex gap-4 mt-12">
            ...
            <button (click)="nextStep()" 
                    [disabled]="!canProceed()"
                    [class]="'flex-[2] py-5 rounded-2xl text-xs font-black transition-all uppercase tracking-widest ' + (canProceed() ? 'bg-navy-900 text-white shadow-2xl hover:bg-teal-600 dark:bg-teal-600 dark:hover:bg-teal-700' : 'bg-slate-100 text-slate-300 dark:bg-slate-800 dark:text-slate-600 cursor-not-allowed')">
            ...
            @if (recommendedTour(); as tour) {
              ...
              <div class="flex flex-col items-center mb-8">
                 <svg class="w-16 h-16 mb-4" viewBox="0 0 52 52">
                   ...
                   <path class="checkmark-check" fill="none" d="M14.1 27.2l7.1 7.2 16.7-16.8" 
                         style="stroke: #0D9488; stroke-width: 4; stroke-linecap: round; stroke-dasharray: 48; stroke-dashoffset: 48; animation: stroke 0.3s cubic-bezier(0.65, 0, 0.45, 1) 0.8s forwards;"/>
                 </svg>
                 <div class="text-xs font-black text-teal-600 uppercase tracking-[0.4em] animate-pulse">Match Found</div>
              </div>
              ...
              <div class="relative rounded-[2rem] overflow-hidden mb-8 group bg-navy-900 shadow-2xl ring-4 ring-teal-600/10">
                ...
                <div class="absolute bottom-6 left-6 right-6 text-left">
                  <div class="inline-block bg-teal-600 text-white text-[9px] font-black uppercase tracking-[0.3em] px-3 py-1 rounded-full mb-3">99% Personal Match</div>
