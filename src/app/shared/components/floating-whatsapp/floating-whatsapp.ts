...
@Component({
  ...
  template: `
    <a [href]="whatsappLink()" 
       target="_blank"
       class="fixed bottom-6 right-6 z-50 flex items-center justify-center w-16 h-16 bg-green-500 rounded-full shadow-2xl hover:bg-green-600 transition-all duration-300 hover:scale-110 group animate-bounce">
      ...
      <span class="absolute right-full mr-4 px-3 py-1 bg-white text-gray-800 text-xs font-semibold rounded-lg shadow-xl border border-gray-100 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
